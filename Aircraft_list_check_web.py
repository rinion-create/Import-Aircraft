...
- Master Excel: auto-resolve from OneDrive (tries common OneDrive folder names and both 'Aicraft'/'Aircraft' spellings),
...
- Outputs:
    ...
    - Easy download buttons (and optional OneDrive save path if desired)
...
def pick_column(df: pd.DataFrame, candidates) -> str:
    ...
    raise ValueError(f"Could not auto-detect any of {candidates}. Available: {list(df.columns)}")


def try_pick_column(df: pd.DataFrame, candidates) -> Optional[str]:
    try:
...
def process_import(
    ...
    info, debug,
    orig_stem: str,  # NEW: original filename stem from uploaded import
):
...
st.caption("Ruben Inion v0.1 2026.")
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("Master override (optional)")
    master_upload = st.file_uploader("Upload Master Excel (.xlsx) to override OneDrive auto-detection", type=["xlsx"])
...
if master_upload is not None:
    ...
    # Try OneDrive auto-resolution
    candidates = master_path_candidates()
    existing = [p for p in candidates if os.path.exists(p)]
    if existing:
        try:
            master_dict = load_master_from_path(existing[0], info, debug)
            master_status_lines.append(f"Master auto-detected: `{existing[0]}`")
        except Exception as e:
            st.error(f"Failed to read detected Master: {e}")
    else:
        master_status_lines.append("No OneDrive Master found. You can upload a Master file in the sidebar.")
...
if run_btn and import_upload is not None:
    try:
        ...
        if master_dict is None:
            st.error("Master dataset is not available. Please upload a Master file or ensure OneDrive path exists.")
        else:
